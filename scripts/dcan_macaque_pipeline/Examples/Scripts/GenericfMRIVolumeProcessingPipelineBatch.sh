...
#
#	${StudyFolder}/unprocessed/3T/tfMRI_EMOTION_PA/${Subject}_3T_tfMRI_EMOTION_PA.nii.gz
#	${StudyFolder}/unprocessed/3T/tfMRI_EMOTION_PA/${Subject}_3T_tfMRI_EMOTION_PA_SBRef.nii.gz
#
#	${StudyFolder}/unprocessed/3T/tfMRI_EMOTION_AP/${Subject}_3T_tfMRI_EMOTION_AP.nii.gz
#	${StudyFolder}/unprocessed/3T/tfMRI_EMOTION_AP/${Subject}_3T_tfMRI_EMOTION_AP_SBRef.nii.gz
#
#	${StudyFolder}/unprocessed/3T/tfMRI_EMOTION_PA/${Subject}_3T_SpinEchoFieldMap_PA.nii.gz
#	${StudyFolder}/unprocessed/3T/tfMRI_EMOTION_PA/${Subject}_3T_SpinEchoFieldMap_AP.nii.gz
#
#	${StudyFolder}/unprocessed/3T/tfMRI_EMOTION_AP/${Subject}_3T_SpinEchoFieldMap_PA.nii.gz
#	${StudyFolder}/unprocessed/3T/tfMRI_EMOTION_AP/${Subject}_3T_SpinEchoFieldMap_AP.nii.gz
#
#
...
for Subject in $Subjlist ; do
  ...
  if [[ ! -z "$command_line_specified_task" ]]; then
    ...
    Tasklist=""
    # Ensure ffmri runs after bold data in sequential case.
    numrest=$(ls "${StudyFolder}/unprocessed/NIFTI" | grep REST | grep -v ffMRI | wc -l) # ensure ffmri data is processed last in serial case.
    numfer=$(ls "${StudyFolder}/unprocessed/NIFTI" | grep ffMRI_REST | wc -l)
    for i in `seq 1 $numfer`; do
        Tasklist="ffMRI_REST$i $Tasklist"
    done
    for i in `seq 1 $numrest`; do
    	# legacy naming
    	if ls "${StudyFolder}/unprocessed/NIFTI" | grep rfMRI_REST > /dev/null; then
            Tasklist="rfMRI_REST$i $Tasklist"
        else
            Tasklist="REST$i $Tasklist"
        fi
    done
  fi
  i=1
  for fMRIName in $Tasklist ; do
    ...
    fMRITimeSeries="${StudyFolder}/unprocessed/3T/${fMRIName}/${Subject}_3T_${fMRIName}.nii.gz"
    fMRISBRef="${StudyFolder}/unprocessed/3T/${fMRIName}/${Subject}_3T_${fMRIName}_SBRef.nii.gz" #A single band reference image (SBRef) is recommended if using multiband, set to NONE if you want to use the first volume of the timeseries for motion correction
    if ${Ferumoxytol:-false} && [[ $fMRIName =~ .*ffmri.* ]]; then
        useBoldScout=true # flag to use bold epi reg for fer registration...
    fi
    ...
    SpinEchoPhaseEncodeNegative="${StudyFolder}/unprocessed/3T/${fMRIName}/${Subject}_3T_SpinEchoFieldMap_LR.nii.gz" #For the spin echo field map volume with a negative phase encoding direction (LR in HCP data, AP in 7T HCP data), set to NONE if using regular FIELDMAP
    SpinEchoPhaseEncodePositive="${StudyFolder}/unprocessed/3T/${fMRIName}/${Subject}_3T_SpinEchoFieldMap_RL.nii.gz" #For the spin echo field map volume with a positive phase encoding direction (RL in HCP data, PA in 7T HCP data), set to NONE if using regular FIELDMAP
    ...
    # Reverse EPI Protocol
    if ${ReverseEpi:-false}; then
      SpinEchoPhaseEncodeNegative=${StudyFolder}/$Subject/unprocessed/NIFTI/${Subject}_ReversePhaseEncodeEPI.nii.gz
    fi
    ...
    #
    # Example Value: 
    #  GEB0InputName="${StudyFolder}/unprocessed/3T/${fMRIName}/${Subject}_3T_GradientEchoFieldMap.nii.gz"
    GEB0InputName="NONE"
    ...
    ${queuing_command} ${HCPPIPEDIR}/fMRIVolume/GenericfMRIVolumeProcessingPipeline.sh \
      ...
      --fmapmagbrain="$StudyFolder"/masks/$(basename $(remove_ext "$MagnitudeInputName"))_brain.nii.gz `# kludgy method for now` \
      ...
      --mctype=${MCType} \
      --useT2=${useT2} \
      --userevepi=${ReverseEpi:-false} \
      --useboldscout=${useBoldScout:-false}
