...
def argument_parse_gradunwarp():
    ...
    # required arguments
    p.add_argument("infile", action="store", help="The input warped file (nifti or mgh)")
    p.add_argument(
        "outfile", action="store", help="The output unwarped file (extension should be .nii/.nii.gz/.mgh/.mgz)"
    )
    p.add_argument(
        "vendor", action="store", choices=["siemens", "ge"], help='vendor (either "ge" or "siemens" for now)'
    )

    coef_grp = p.add_mutually_exclusive_group(required=True)
    coef_grp.add_argument("-g", "--gradfile", dest="gradfile", help="The .grad coefficient file")
    coef_grp.add_argument("-c", "--coeffile", dest="coeffile", help="The .coef coefficient file")

    # optional arguments
    p.add_argument("-w", "--warp", action="store_true", default=False, help="warp a volume (as opposed to unwarping)")
    p.add_argument(
        "-n",
        "--nojacobian",
        dest="nojac",
        action="store_true",
        default=False,
        help="Do not perform Jacobian intensity correction",
    )
    p.add_argument("--fovmin", dest="fovmin", help="the minimum extent of harmonics evaluation grid in meters")
    p.add_argument("--fovmax", dest="fovmax", help="the maximum extent of harmonics evaluation grid in meters")
    p.add_argument("--numpoints", dest="numpoints", help="number of grid points in each direction")
    p.add_argument("--interp_order", dest="order", help="the order of interpolation(1..4) where 1 is linear - default")
...
class GradientUnwarpRunner(object):
    ...
    def run(self):
        ...
        if hasattr(self.args, "gradfile") and self.args.gradfile:
            self.coeffs = coeffs.get_coefficients(self.args.vendor, self.args.gradfile)
        else:
