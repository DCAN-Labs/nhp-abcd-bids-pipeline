...
# default parameters
#ERIC:REMOVING HARDCODING Reference=`defaultopt ${Reference} ${FSLDIR}/data/standard/MNI152_T1_1mm`
...
### ERIC: acpc.mat should be a linear transform from the original to the atlas (human ACPC does not align in monkeys, so instead the full2std.mat is directly copied to acpc.mat) ###
#cp -f "$WD"/full2std.mat "$OutputMatrix" ### ERIC REPLACEMENT FOR MONKEYS ###
