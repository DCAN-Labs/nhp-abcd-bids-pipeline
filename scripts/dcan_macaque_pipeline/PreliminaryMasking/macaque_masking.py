...
def generate_parser(parser=None):
    ...
    parser.add_argument("--path", type=str, help="path to study folder for subject")
    parser.add_argument("--t1", nargs="+", help="space delimited input t1w image paths")
    parser.add_argument("--t2", nargs="+", help="space delimited input t2w image paths")
    parser.add_argument("--func", nargs="+", help="space delimited input functional data")
    parser.add_argument("--fmapmag", help="diffusion field map path if it is used.")
    parser.add_argument(
        "--sshead", help="study atlas head for ants masking, should generally match age " "and gender for best results"
    )
    parser.add_argument(
        "--ssbrain",
        help="study atlas brain for ants masking, should generally match age " "and gender for best results",
    )
    parser.add_argument(
        "--t1brainmask",
    ...
    parser.add_argument("--verbose", action="store_true", help="print commands prior to execution")
...
def interface(
    ...
    kwargs = {
        ...
        "warp_files": " ".join((os.path.join(wd, "atl2T1Warp.nii.gz"), os.path.join(wd, "atl2T1Affine.txt"))),
    ...
    # average images
    for txw_images, avg in ((t1w_images, kwargs["t1w"]), (t2w_images, kwargs["t2w"])):
        if len(txw_images) > 1:
            avg_cmd = "{}/better_flirt_average {} {} {}".format(
                os.environ["HCPPIPEDIR_Global"], len(txw_images), " ".join(txw_images), avg
            )
        else:
    ...
    if t1_brain_mask and t1_brain_mask.upper() != "NONE":
        ...
        copy_t1w_mask = "cp {user_t1w_mask} {brain_mask}".format(**kwargs)
        user_mask_t1w = "fslmaths {t1w} -mas {brain_mask} {t1w_brain}".format(**kwargs)
        ...
        rigid_align = (
            "{FSLDIR}/bin/flirt -v -dof 6 -in {t1w} -ref {atlas_head} "
            "-out {t1w_in_atl} -omat {t1w2atl} -interp spline "
            "-searchrx -30 30 -searchry -30 30 -searchrz -30 "
            "30".format(**kwargs)
        )
        create_mask = "{FSLDIR}/bin/fslmaths {atlas_brain} -bin {" "atlas_mask}".format(**kwargs)
        ants_warp = (
            "{ANTSPATH}/ANTS 3 -m  CC[{t1w_in_atl},{atlas_head},1,5] "
        ...
        apply_ants_warp = (
            "{ANTSPATH}/antsApplyTransforms -d 3 -i {atlas_mask} "
            "-t {warp_files} -r {t1w_in_atl} -o "
            "{warp_mask}".format(**kwargs)
        )
        inverse_mat = "{FSLDIR}/bin/convert_xfm -omat {atl2t1w} -inverse {" "t1w2atl}".format(**kwargs)
        rigid_align_mask = (
            "{FSLDIR}/bin/flirt -interp nearestneighbour -in {"
    ...
    if t2_brain_mask and t2_brain_mask.upper() != "NONE":
        # if a user-specified mask exists, skip ants mask generation
        # and apply the specified mask instead
        copy_t2w_mask = "cp {user_t2w_mask} {t2w_brain_mask}".format(**kwargs)
        user_mask_t2w = "fslmaths {t2w} -mas {t2w_brain_mask} {t2w_brain}".format(**kwargs)
        ...
        t1w2t2w_rigid = "flirt -dof 6 -cost mutualinfo -in {t1w} -ref {t2w} " "-omat {t1w2t2w}".format(**kwargs)
        t1w2t2w_mask = (
            "flirt -in {brain_mask} -interp nearestneighbour -ref {"
            "t2w} -o {t2w_brain_mask} -applyxfm -init {"
            "t1w2t2w}".format(**kwargs)
        )
        mask_t2w = "fslmaths {t2w} -mas {t2w_brain_mask} " "{t2w_brain}".format(**kwargs)
        cmdlist += [bias_field_correct_t2w, t1w2t2w_rigid, t1w2t2w_mask, mask_t2w]

    # create fieldmap mask and bet mask if applicable
    if dfm_image and dfm_image.upper() != "NONE":
        rigid_dfm = "flirt -dof 6 -in {t2w} -ref {dfm_image} -omat {" "t2w2dfm}".format(**kwargs)
        align_mask = (
            "flirt -interp nearestneighbour -in {t2w_brain_mask} "
            "-ref {dfm_image} -applyxfm -init {t2w2dfm} -o "
            "{dfm_brain_mask}".format(**kwargs)
        )
        apply_mask = "fslmaths {dfm_image} -mas {dfm_brain_mask} {" "dfm_brain}".format(**kwargs)
        ...
        for func in functional_images:
            ...
            func_bet = os.path.join(wd, os.path.basename(func))[:-7] + "_bet.nii.gz"
            func_mask = func_bet[:-7] + "_mask.nii.gz"
            bet = "{FSLDIR}/bin/bet {func} {func_bet} -m -f 0.25 -g " "0.3".format(
                **kwargs, func=func, func_bet=func_bet, func_mask=func_mask
            )
            cmdlist += [bet]
