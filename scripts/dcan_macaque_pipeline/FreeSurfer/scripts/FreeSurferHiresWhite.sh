...
tkregister2 --mov "$mridir"/T1w_hires.nii.gz --targ $mridir/orig.mgz --noedit --regheader --reg $reg #pointing to freesurfer 53, OMD
...
# Feczko's code for fixing the .mgzs
sizex=`fslinfo ${mridir}/T1w_hires.nii.gz | awk '$1=="dim1" {print $2}'`
sizey=`fslinfo ${mridir}/T1w_hires.nii.gz | awk '$1=="dim2" {print $2}'`
sizez=`fslinfo ${mridir}/T1w_hires.nii.gz | awk '$1=="dim3" {print $2}'`
pixsizex=`fslinfo ${mridir}/T1w_hires.nii.gz | awk '$1=="pixdim1" {print $2}'`
pixsizey=`fslinfo ${mridir}/T1w_hires.nii.gz | awk '$1=="pixdim2" {print $2}'`
pixsizez=`fslinfo ${mridir}/T1w_hires.nii.gz | awk '$1=="pixdim3" {print $2}'`
#-----------------------------------------
#Now get the sform from the hires image:
newsform=`fslorient -getsform ${mridir}/T1w_hires.nii.gz`
#-----------------------------------------
for MRIimage in wm aseg brain filled; do
mri_convert --in_orientation LIA --out_orientation RAS --cropsize ${sizex} ${sizey} ${sizez} ${mridir}/${MRIimage}.mgz ${mridir}/${MRIimage}_crop.nii.gz -odt float;
fslchpixdim ${mridir}/${MRIimage}_crop.nii.gz ${pixsizex} ${pixsizey} ${pixsizez};
fslorient -setsform ${newsform} ${mridir}/${MRIimage}_crop.nii.gz;
mri_convert --force_ras_good ${mridir}/${MRIimage}_crop.nii.gz ${mridir}/${MRIimage}.hires.mgz;
done

# replaced by Feczko's script above:F
...
# just copy T2w which is already aligned in the case that distortion correction already ran. - Darrick
if [ -e ${SubjectDIR}/../T2w/ACPCAlignment ]; then
  cp ${SubjectDIR}/T2w_acpc_dc_restore_brain.nii.gz "$mridir"/T2w_hires.nii.gz
