...
/usr/global/freesurfer53/bin/tkregister2 --mov "$mridir"/T1w_hires.nii.gz --targ $mridir/orig.mgz --noedit --regheader --reg $reg #pointing to freesurfer 53, OMD
# map white and pial to hires coords (pial is only for visualization - won't be used later)
## removed because these copies don't appear to do anything good, only something very bad -Darrick
...
/usr/global/freesurfer53/bin/mri_surf2surf --s $SubjectID --sval-xyz white --reg $reg "$mridir"/T1w_hires.nii.gz --tval-xyz --tval white.hires --hemi lh
/usr/global/freesurfer53/bin/mri_surf2surf --s $SubjectID --sval-xyz white --reg $reg "$mridir"/T1w_hires.nii.gz --tval-xyz --tval white.hires --hemi rh
...
for v in wm.mgz filled.mgz brain.mgz aseg.mgz ; do
  ...
  /usr/global/freesurfer53/bin/mri_convert -rl "$mridir"/T1w_hires.nii.gz -rt nearest $mridir/$v $mridir/$basename.hires.mgz
done

# remove nonbrain tissue
/usr/global/freesurfer53/bin/mri_mask "$mridir"/T1w_hires.nii.gz $mridir/brain.hires.mgz $mridir/T1w_hires.masked.mgz

/usr/global/freesurfer53/bin/mri_convert $mridir/aseg.hires.mgz $mridir/aseg.hires.nii.gz
...
/usr/global/freesurfer53/bin/mri_normalize -erode 1 -f $SubjectDIR/$SubjectID/scripts/control.hires.dat -min_dist 1 -surface "$surfdir"/lh.white.hires identity.nofile -surface "$surfdir"/rh.white.hires identity.nofile $mridir/T1w_hires.masked.mgz $mridir/T1w_hires.masked.norm.mgz

#Check if FreeSurfer is version 5.2.0 or not.  If it is not, use new -first_wm_peak mris_make_surfaces flag
#Hey, they saifd that this could run on 5.2 or higher, OMD
...
#deform the surfaces
/usr/global/freesurfer53/bin/mris_make_surfaces ${FIRSTWMPEAK} -noaparc -aseg aseg.hires -orig white.hires -filled filled.hires -wm wm.hires -sdir $SubjectDIR -T1 T1w_hires.masked.norm -orig_white white.hires -output .deformed -w 0 $SubjectID lh
/usr/global/freesurfer53/bin/mris_make_surfaces ${FIRSTWMPEAK} -noaparc -aseg aseg.hires -orig white.hires -filled filled.hires -wm wm.hires -sdir $SubjectDIR -T1 T1w_hires.masked.norm -orig_white white.hires -output .deformed -w 0 $SubjectID rh
...
if [ ! -e "$mridir"/transforms/T2wtoT1w.mat ] ; then
  /usr/global/freesurfer53/bin/bbregister --s "$SubjectID" --mov "$T2wImage" --surf white.deformed --init-reg "$mridir"/transforms/eye.dat --t2 --reg "$mridir"/transforms/T2wtoT1w.dat --o "$mridir"/T2w_hires.nii.gz
  /usr/global/freesurfer53/bin/tkregister2 --noedit --reg "$mridir"/transforms/T2wtoT1w.dat --mov "$T2wImage" --targ "$mridir"/T1w_hires.nii.gz --fslregout "$mridir"/transforms/T2wtoT1w.mat
...
/usr/global/freesurfer53/bin/tkregister2 --mov $mridir/orig.mgz --targ "$mridir"/T1w_hires.nii.gz --noedit --regheader --reg $regII
/usr/global/freesurfer53/bin/mri_surf2surf --s $SubjectID --sval-xyz white.deformed --reg $regII $mridir/orig.mgz --tval-xyz --tval white --hemi lh
/usr/global/freesurfer53/bin/mri_surf2surf --s $SubjectID --sval-xyz white.deformed --reg $regII $mridir/orig.mgz --tval-xyz --tval white --hemi rh
