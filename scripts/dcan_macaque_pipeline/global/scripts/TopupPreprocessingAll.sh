...
# check if mask if > 1 frame
# we need to make it a single frame else fslmaths will error
MASK_T_NUM=$(fslinfo ${WD}/Mask.nii.gz | grep dim4 | head -n 1 | awk -F' ' '{ print $2}')
[[ $MASK_T_NUM -gt 1 ]] && fslroi ${WD}/Mask.nii.gz ${WD}/Mask.nii.gz 0 1
