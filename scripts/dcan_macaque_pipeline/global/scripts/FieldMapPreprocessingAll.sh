...
if [ -z "$MagnitudeInputBrainName" ]; then
  echo "Should not appear in Macaque DFM pipeline"
  ${FSLDIR}/bin/bet ${WD}/Magnitude ${WD}/Magnitude_brain -f 0.35 -m #Brain extract the magnitude image
elif [ -e "$MagnitudeInputBrainName" ]; then
  ${FSLDIR}/bin/fslmaths "$MagnitudeInputBrainName" -Tmean -bin ${WD}/Magnitude_mask
  ${FSLDIR}/bin/fslmaths ${WD}/Magnitude -mas ${WD}/Magnitude_mask ${WD}/Magnitude_brain
else
  (>&2 echo "$MagnitudeInputBrainName" does not exist)
  exit 1
fi
