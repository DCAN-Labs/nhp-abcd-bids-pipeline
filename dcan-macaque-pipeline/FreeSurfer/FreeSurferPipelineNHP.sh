...
useT2=`getopt1 "--useT2" $@` # grab useT2 flag from batch file

T1wImageFile=`remove_ext $T1wImage`;
T1wImageBrainFile=`remove_ext $T1wImageBrain`;

if $useT2; then T2wImageFile=`remove_ext $T2wImage`; fi
...
if $useT2; then
"$PipelineScripts"/FreeSurferHiresPial.sh "$SubjectID" "$SubjectDIR" "$T1wImageFile"_1mm.nii.gz "$T2wImageFile"_1mm.nii.gz
fi
