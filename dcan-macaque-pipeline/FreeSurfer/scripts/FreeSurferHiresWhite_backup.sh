...
useT2="$5" #useT2 flag included by Anders to skip fine tuning of T2w to T1w registration
...
# freesurfer dir declared -DARRICK
/usr/global/freesurfer53/bin/mris_make_surfaces ${FIRSTWMPEAK} -whiteonly -noaparc -aseg aseg.hires -orig white.hires -filled filled.hires -wm wm.hires -sdir $SubjectDIR -T1 T1w_hires.masked.norm -orig_white white.hires -output .deformed -w 0 $SubjectID lh
/usr/global/freesurfer53/bin/mris_make_surfaces ${FIRSTWMPEAK} -whiteonly -noaparc -aseg aseg.hires -orig white.hires -filled filled.hires -wm wm.hires -sdir $SubjectDIR -T1 T1w_hires.masked.norm -orig_white white.hires -output .deformed -w 0 $SubjectID rh
...
if $useT2; then #allow generation of eye.dat (identity matrix) necessary in fMRIVol - Anders Perrone 10/17/2016
